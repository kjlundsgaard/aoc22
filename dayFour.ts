...
const findOverlap = (assignmentOne: string, assignmentTwo: string): boolean => {
  let doesOverlap = false;
  const assOneBounds = getBounds(assignmentOne);
  const assTwoBounds = getBounds(assignmentTwo);
  if (
    (assOneBounds.start >= assTwoBounds.start &&
...
const getBounds = (section: string): { [key: string]: number } => {
...
const run = () => {
  const input = parseFile(`inputs/dayFour.txt`);
