...
import _, { constant, result, size } from "lodash";
...
const fileSizeMap: { [key: string]: number } = {};
const countFileSize = () => {
...
const countFileSizeForDir = (dir: string) => {
  ...
  childDirs.forEach((childDir: string) => {
...
const buildLayout = (input: string[]) => {
  let currentDir = "/";
...
const filterFilesBySize = (size: number) => {
...
const calculateTotalSize = (fileMap: any) => {
...
const sortFilesBySpace = (fileMap: { [key: string]: number }) => {
...
const run = () => {
  const input = parseFile(`inputs/daySeven.txt`);
  buildLayout(input);
  countFileSize();
